...
@app.route("/webservice/universal")
def universal():
    web = ref.child("Website").child("universalVARR").get()
    return web

@app.route("/webservice/notifications")
def notifications():
    web = ref.child("Website").child("notifications").get()
    return web

@app.route("/webservice/blogs")
def Blogs():
    web = ref.child("Blogs").get()
    return str(web).replace("'",'"')

@app.route("/getBlog/<id>")
def GetBlog(id):
    web = json.dumps(ref.child("Blog").child(id).get())
    return str(web)
